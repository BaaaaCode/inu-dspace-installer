...
EOF
    ...
    # 메모리 확보: 빌드 중 불필요한 서비스 임시 중지
    systemctl stop dspace-backend || true

    cd /opt/dspace-angular
    sudo -u ${DSPACE_USER} npm install
    # Node.js 힙 크기 확대 (OOM 방지)
    sudo -u ${DSPACE_USER} bash -c "export NODE_OPTIONS='--max-old-space-size=2048' && cd /opt/dspace-angular && npm run build:prod"
