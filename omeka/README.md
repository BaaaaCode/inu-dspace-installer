...
[<SSH_GATEWAY>:<GATEWAY_PORT>]  ← SSH 게이트웨이
    │
    ▼
[물리 서버 <SERVER_IP>]
    │ VirtualBox NAT
    ├── :20260 → <MASTER_VM> (관리 서버)
    ├── :20272 → IS20272
    ├── :20291 → <OMEKA_VM> (Omeka 테스트)
    └── ...
```
...
```
직접 접근 (안 됨):
  브라우저 → <SERVER_IP>:20291 → SSH 서비스가 받음 → ❌
...
| OS | Ubuntu 22.04 LTS |
| 물리 서버 IP | <SERVER_IP> |
| SSH 게이트웨이 | <SSH_GATEWAY>:<GATEWAY_PORT> |
| TA 테스트 포트 | 20291 (<OMEKA_VM>) |
...
| Type | Local |
| Local port | `20291` |
| Destination | `<SERVER_IP>:20291` |
| SSH server | `<USER>@<SSH_GATEWAY>:<GATEWAY_PORT>` |
...
| Type | Local |
| Local port | `20265` |
| Destination | `<SERVER_IP>:20265` |
| SSH server | `본인계정@<SSH_GATEWAY>:<GATEWAY_PORT>` |
...
==================================================
  ...
  환경 모드     : local
  공개용 접속   : http://<SERVER_IP>:20265
  관리자 페이지 : http://<SERVER_IP>:20265/admin
```
...
> SSH 터널링을 통해 포워딩된 것이므로, `<SERVER_IP>`가 아닌 **`localhost`**로 접근해야 합니다.
...
|------|-----|------|---------------------|
| TA (테스트) | <OMEKA_VM> | 20291 | `http://localhost:20291/` |
| 학생 A | <STUDENT_A_VM> | 20265 | `http://localhost:20265/` |
| 학생 B | <STUDENT_B_VM> | 20266 | `http://localhost:20266/` |
