...
###############################################################################
# 6. Solr 코어 복사 + core.properties 생성
###############################################################################
section "SOLR CORES"
...
curl -s http://localhost:8983/solr/admin/cores?action=STATUS | grep -q '"search"' \
    && echo "Solr cores: OK" || echo "WARNING: Solr cores may not be loaded"
