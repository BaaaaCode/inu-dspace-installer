...
# Solr Home 경로 확인
SOLR_HOME="/opt/solr-${SOLR_VERSION}/server/solr"
...
cd ${INSTALL_DIR}
if [ -f "${DSPACE_DIR}/webapps/server-boot.jar" ]; then
    echo "Backend already built. Skipping download & build."
else
...
###############################################################################
# 6. Solr 코어 복사 (Solr Home 경로로) + core.properties 생성
###############################################################################
section "SOLR CORES"
cp -r ${DSPACE_DIR}/solr/* ${SOLR_HOME}/

# core.properties에 이름 명시 (비어있으면 Solr가 코어를 인식하지 못함)
...
# Solr 코어 등록 확인 (최대 30초 대기)
echo "Waiting for Solr cores to load..."
for i in $(seq 1 6); do
    sleep 5
    if curl -s http://localhost:8983/solr/admin/cores?action=STATUS | grep -q '"search"'; then
        echo "Solr cores: OK (loaded in $((i*5))s)"
        break
    fi
    if [ $i -eq 6 ]; then
        echo "ERROR: Solr cores not loaded after 30s"
        exit 1
    fi
    echo "  retry $i/6..."
done
...
if [ -d "/opt/dspace-angular/dist" ]; then
    echo "Frontend already built. Skipping."
else
    # 다운로드 (소스가 없을 때만)
    if [ ! -f "/opt/dspace-angular/package.json" ]; then
...
EOF
        ...
        echo "Frontend source exists. Skipping download."
    fi

    # npm install (node_modules 없을 때만)
    cd /opt/dspace-angular
    if [ ! -d "/opt/dspace-angular/node_modules" ]; then
    ...
    # 메모리 확보: 빌드 중 불필요한 서비스 모두 중지
    systemctl stop dspace-backend || true
    systemctl stop solr || true

    # Node.js 힙 4GB로 확대 (OOM 방지)
    sudo -u ${DSPACE_USER} bash -c "export NODE_OPTIONS='--max-old-space-size=4096' && cd /opt/dspace-angular && npm run build:prod"
